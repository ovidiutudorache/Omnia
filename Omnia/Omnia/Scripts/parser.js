...
var Parser = (function (scope) {
    ...
    Expression.prototype = {
        simplify: function (values) {
            ...
            for (i = 0; i < L; i++) {
                ...
                if (type_ === TNUMBER) {
                    ...
                    var value = values[item.index_];
                    //var type = isNaN(value) ? TVAR : TNUMBER;
                    var type = TNUMBER;
                    item = new Token(type, 0, 0, value);
                    item.Delta = delta;
                    nstack.push(item);
                }
        ...
        evaluate: function (values) {
            ...
            for (i = 0; i < L; i++) {
                ...
                var type_ = item.type_;
                if (type_ === TNUMBER) {
                    if (item.number_ && item.Delta) {
                        var path = item.Delta.split('.');
                        var value = item.number_;
    ...
    Parser.parse = function (expr, model) {
        var p = new Parser();

        return p.parse(expr, model, p);
    };
        ...
        parse: function (expr, model, p) {
            ...
            while (this.pos < this.expression.length) {
                if (this.isOperator()) {
                    ...
                    if (model && p && (this.tokenindex in model) && this.expression.length > this.pos + 1) {
                        this.pos++;
                        var delta = '';
                        while (this.expression.length > this.pos && (this.expression[this.pos] == '.' || p.isAlphaNumeric(this.expression[this.pos]))) {
                            delta += this.expression[this.pos];
                            this.pos++;
                        }

                        if (delta.length)
                            vartoken.Delta = delta;
                    }
